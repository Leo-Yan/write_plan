...
Because multiple events share the same ring buffer for sampling, when any event sample is recorded into the ring buffer, the kernel event core layer simply wakes up sleeping tasks related to the ring buffer.  This is fulfilled by the kernel function \code{ring\_buffer\_wakeup()} which iterates every event associated to the ring buffer and wakes up tasks on the wait queue of the events.
