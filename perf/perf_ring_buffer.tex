...
\subsection*{Snapshot mode for AUX ring buffer}
...
\begin{itemize}
	...
	\item When the user wants to snapshot it signals \code{USR2} to perf tool, perf will invoke the callback \code{auxtrace\_record::snapshot\_start()} to disable the hardware tracing;
	\item \code{record\_\_read\_auxtrace\_snapshot()} reads out the hardware trace data from AUX ring buffer, and saves into perf data file; during this process, the callback \code{auxtrace\_record::find\_snapshot()} is used to calculate the head and old head (these two heads are used to cauculate the trace data size which will be described soon);
