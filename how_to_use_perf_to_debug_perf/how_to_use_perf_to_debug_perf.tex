...
The \code{'perf probe'} command with option \code{'---line'} dumps which code
lines in \code{perf\_aux\_output\_end()} are suitable for injecting a dynamic
tracepoint:
...
perf probe --line "perf_aux_output_end" -k /path/to/vmlinux -s /path/to/kernel/
\end{lstlisting}

Perf needs the vmlinux image. If the vmlinux path is not specified by the option
...
source, which should be consistent with the built kernel image.

\begin{lstlisting}[
  	language=sh,
	breaklines=true
]
perf probe --line "perf_aux_output_end" -s /mnt/linux-kernel
...
In the annotation, the lines with prefixed numbers are candidates for probes.
The new buffer head is calculated from the line 8 to line 16. After that, the
line 31 is a good place to observe the result.

Next, we intend to check which variables are available in the line 31 of the
function. So the option \code{'---vars'} can be used at this point:
...
In the log, \code{'@<perf\_aux\_output\_end+68>'} indicates that the probe
point has the offset of 68 from the start of the function. Then, there are
five local variables availiable for tracing. If we recall the previous
analysis, we know that \code{aux\_head} holds the old head,
\code{rb->aux\_head} is for the new head, and the variable \code{size} is
for the filled size.
...
The rest thing is to apply the same steps for adding a probe in the perf tool.

Firstly, we display the source code lines for the \code{\_\_auxtrace\_mmap\_\_read()} function:
...
When you try to
show variables for an inline function, it's even possible to output multiple
probe points. This is because an inline function can be linked into different
functions, so the linked functions plus offset will be displayed.

\begin{lstlisting}[
  	language=sh,
	breaklines=true
]
perf record -o perf.data.dbg -e probe:perf_aux_output_end_L31 -e probe_perf:__auxtrace_mmap__read -- perf record -o perf.data.etm -e cs_etm// -- ls
...
perf record -o perf.data.dbg -e probe:perf_aux_output_end_L31/call-graph=fp/ -e probe_perf:__auxtrace_mmap__read/call-graph=fp/ -- perf record -o perf.data.etm -e cs_etm// -- ls
\end{lstlisting}
