...
acquire intense knowledge before using them.

This is why we move eyes to the perf tool. The perf tool is capable of reading
ELF files and annotating with source code. It provides a much more
user-friendly way to set up a probe.
...
Let's replicate the experiment we have done.
...
suitable for injecting a tracepoint, we can use the \code{'perf probe'}
command with option \code{'---line'}.

Perf needs the vmlinux file. If the vmlinux path is not offered by the option
\code{'-k'} or \code{'---vmlinux'}, it is assumed to be placed in the same
folder as the current working directory or in any of the predefined paths in
the structure
...
The advantage of using perf is that it converts the readable expression to the
probe syntax with raw addresses and registers, which can significantly
reduce the difficulty of setting up probes.
...
\code{\_\_auxtrace\_mmap\_\_read()} function. Additionally, we need to specify
path to the executable with the option \code{'-x'} or \code{'---exec'} for the
user space program.
...
The dumping below shows that the size and heads calculation extends until line
36, and line 38 would be a suitable location for injecting a probe.
...
We also need to confirm which variables are accessible at the line 38 of the
function with option \code{'---vars'}.
...
variables for each offset. Due to line 38 having multiple conditions compiled
with multiple branch instructions, if we add probes for this line, perf will
inject tracepoints for all relevant offsets.
...
\code{'function\_name+offset'} to specify a more accurate probe point. Here,
we select the offset \code{488} for tracing and dump the local variables
\code{head\_off}, \code{old\_off} and \code{size}. The command is as: 
...
After using the perf tool to setup probes, now we need to check how to consume
these tracepoints. Let's begin by reproducing the steps via the tracing file
system.
...
Actually, after we enable the perf, it's no need to use ftrace knobs for the
tracing. The perf tool can handle everything, including recording and
reporting the trace data.

Using perf to debug perf involves two perf programs running. The first program
is to open \code{cs\_etm} PMU event and record Arm CoreSight hardware
trace data for program \code{ls}. The command is
...
The second perf program opens the named probes and takes the first perf
program as its target program. And use another separator \code{'---'}
to divide the two perf programs.

By default, perf uses the name \code{'perf.data'} for a recording if a data file name
is not given. But, if two perf recordings save their trace data into the same
file, it can cause mess. Therefore, we specify distinct output file names
...
With the above explanation, writing the following command to use perf for
debugging perf should not be difficult:
...
The \code{'perf script'} command can help us to print out the trace data.
It can output the details for the events produced by tracepoints. As the
result, we obtain logs in the format below, which is same with ftrace's
dumping format.
...
Sometimes, we are curious about how a function is invoked in a flow. We can
append the configuration 'call-graph=fp' for a tracepoint. When the
tracepoint is recorded, its call graph will be captured as well. The perf
command is updated for call stack tracing:
...
When you've reached at this point, I hope you now have a brief understanding
of how to use perf for tracing and debugging. I'll stop here.
