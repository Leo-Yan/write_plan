...
We can apply the methodology discussed in the previous section to analyze the
disassembly of the perf tool. Firstly, generate the disassembly with the
command:
...
Thus, we can get the disasembly for \code{\_\_auxtrace\_mmap\_\_read()}.
...
The truncated disassembly piece is for reading the head and the old head, and
calculating the the filled buffer size. The \code{'else'} branch handles the
wrap-around case. In the end, both branches run to the address \code{2079e0}
(see the branch instruction \code{"b 2079e0"} at \code{2079c0}), we can select
it as the probed address.
...
stack. Consequently, we can use the command to inject a user probe:
