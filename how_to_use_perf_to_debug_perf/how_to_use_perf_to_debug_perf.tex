...
tool. Ultimately, the article explains why and how to use perf to debug perf.
\end{abstract}
...
programs: it firstly stops its target program, then it takes chance to check
the context. The \textit{context} can be a software concept, e.g. a thread or
task context, a debugger can read variables from the task's stack or heap. The
\textit{context} can be a hardware context - when a developer is using JTAG
debugger to connect hardware and stop CPU, the hardware registers and memory
can be reviewed.
...
and afterwards developers can use it for analysis program's flow and data
values.

The content is divided to four sections:
\begin{itemize}
	\item Printing;
...
The above code introduces a handy tracing tool: libc's \code{printf()}. Then,
when starting to access the Linux kernel, its equivalent function
\code{printk()} will no longer be strange.

A printing log means an event has occurred, alongside variables can be printed
out - this is perfect to act as tracing.

The printing is reliable in most cases. \code{printk()} can be used safely in
any of contexts without locking in the Linux kernel. This is why even if a
developer has no knowledge for interrupt context, thread context and
bottom-half context (e.g. running in softirq or tasklet), printing still can
work well as a main debugging method.
...
Furthermore, it becomes challenging for using printing to debug a program
which crosses both user space and kernel space. The reason is the
\code{printf()} and \code{printk()} functions store logs in separate buffers,
leading to logs being output in an out-of-order format and not readable.

To resolve this issue, syslog is suggested. A user space program needs to use
\code{syslog()} to replace \code{printf()} for routing logs into syslog
service, as result, all modules supporting syslog can output logs into a
central place. Unfortunately, syslog is not supported in every Linux system,
and many programs (including the perf) doesn't support syslog.
