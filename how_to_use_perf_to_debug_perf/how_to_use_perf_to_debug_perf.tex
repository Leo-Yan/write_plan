...
instruction is \code{"ldr x1, [x20, \#144]"}, as mentioned the \code{x20} is
the pointer for the structure \code{perf\_buffer}, and \code{144} is the
offset of the buffer's head in the structure. As we know, \code{x2} contains
the written buffer size, and it is added to the old buffer head (\code{x1})
for a new buffer head stored into the register \code{x0}. This is accomplished
...
In the command, \code{'p'} means to add a probe, \code{'myprobe'} is the event
name for the probe. The address \code{0xffff8000802a100c} is where the probe
is inserted. The followed arguments are for dumping values. The option
\code{'new\_head=\%x0:x64'} means an argument is named as \code{'new\_head'},
reading value from the register \code{x0} and printing as the 64-bit
hexadecimal type \code{'x64'}.

Actually, it's good to improve readability a bit for the kprobe command. We
can use the 'function name + offset' format to replace the arbitrary
...
Moreover, the probe supports fetching memory with the fetch register and an
offset (the syntax is \code{'+/-offset(REG)'}). In the above case, the old
head is loaded into \code{x1} from the address pointed by \code{x20} plus
\code{144}. Instead of accessing the register \code{x1} for retrieving the
old head, we can use \code{'+144(\%x20)'} for the same purpose. Thus, the
command can be updated as:
...
Likewise the kprobe, ftrace provides a sysfs node \code{uprobe\_events} for
inserting a user space probe. And we can apply the methodology discussed in
the previous section to analyze the disassembly of a program running in user
space.
...
echo 'p /mnt/linux-kernel/linux-cs-dev/tools/perf/perf:0x2079e0 \
...
\section*{Using perf for debugging}
