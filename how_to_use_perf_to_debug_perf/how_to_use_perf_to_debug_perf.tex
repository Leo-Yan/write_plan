...
into the \code{trace\_marker} node in C code, below code is with a bit
tweaking for easier calling.
...
The following experiment is to debug the AUX buffer of perf. The AUX buffer is
a ring buffer designed to store hardware tracing data from components such as
Arm CoreSight, Intel PT, etc. The kernel writes data into the buffer, then
the perf tool reads out data from the buffer.

Sometimes, we need to inspect how the buffer is consumed. The kernel calls the
\code{perf\_aux\_output\_end()} function to update the head of the buffer and
send a notification to user space. The buffer's overwrite mode is only used
for snapshot mode. For simplicity, we can ignore it and solely focus on the
normal case from line 493 to line 498.

In the code below, line 496 saves the old head value into \code{aux\_head}; in
line 497, \code{rb->aux\_head} is updated as a new head by adding the
\code{size} to the old head. For debugging purpose, lines 500 and 501 are
added to print the old head, the new head and the size into ftrace.
...
retrieve the latest head of the buffer. From lines 1869 to 1875, it handles
the overflow case by using a mask or dividing by the buffer length. Finally,
a delta between the old head and the new head is calculated, taking into
account any wrapping around, which is accomplished between line 1877 and
line 1880.

It already contains debugging code at line 1866 for printing logs, but it
only outputs messages to the terminal or log file, we have no chance to print
...
system, and it will be ready for debugging. To prepare ftrace for a fresh
context before running the test, you can use several commands:
...
Based on the log, we can know that the \code{perf\_aux\_output\_end()} function
has been invoked multiple times in the kernel. In the end, the perf tool called
\code{\_\_auxtrace\_mmap\_\_read()} once to read out all trace data from AUX
buffer.

The writing in the kernel and the reading in user space are not paired. It's
apparent that the perf tool is not necessarily waken up every time the kernel
stores trace data. We can easily verify this idea with scheduler tracepoint in
ftrace, which will be discussed in the next section.
...
ftrace has extended to support more tracers, such as latency tracer, etc. The
above log shows "tracer: nop", which means that no tracer is enabled.

The Linux kernel provides tracepoints for debugging and profiling, which are
predefined and called in the kernel source code. These are commonly known as
"static tracepoints". After the system boots up, you can see the available
tracepoints in the subfolder \code{events} or the node \code{available\_events}
under the ftrace's debugfs folder.

Ftrace can combine tracer, tracepoints, and printing together for debugging.

In previous section, the trace is absent to show how the perf tool is waken
up. By enabling the scheduler tracepoints, and combining with printing, we
can get below logs:
...
into AUX buffer. Subsequently, it woke up the perf tool in the
\code{sched\_waking} event. The \(CPU1\) was pulled out from idle, and the
scheduler woke up the perf to run on it.

\section*{Dynamic tracing}

In the previous chapter, we saw that printing and tracepoints in ftrace are
powerful for helping us to understand the code. However, it's not efficient,
as we must rebuild the source code to add print sentences. We will explore
more for dynamic tracing - add tracing without building code.
